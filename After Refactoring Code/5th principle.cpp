...
// **Apply DIP: Patient Should Depend on Abstraction**
class Patient {
private:
    string name;
    int age, id;
    IBedAllocator& bedAllocator;  // Depend on Abstraction
public:
...
private:
    IBedManager& bedManager;  // Depend on abstraction
public:
    HospitalService(IBedManager& manager) : bedManager(manager) {}

    void execute() override {
        int subChoice;
        Patient patient(bedManager); // Depend on abstraction
        do {
...
// **Apply DIP: Inject Services via Constructor**
class HospitalManagementSystem {
private:
    vector<unique_ptr<IService>> services;  // Using unique_ptr for better memory management
public:
    HospitalManagementSystem(vector<unique_ptr<IService>> serviceList) 
        : services(move(serviceList)) {} // Receive dependencies externally
...
int main() {
    unique_ptr<IBedManager> bedManager = make_unique<GeneralBedManager>(500); // Can be replaced with ICUBedManager
