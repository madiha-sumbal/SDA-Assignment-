...
// Interface Segregation for Bed Management

// IBedCapacity - Interface for displaying bed capacity
class IBedCapacity {
public:
    virtual void showCapacity() const = 0;  // Method to show available bed capacity
    virtual ~IBedCapacity() = default;
};

// IBedAllocator - Interface for allocating beds
class IBedAllocator {
public:
    virtual bool allocateBed() = 0;  // Method to allocate a bed
    virtual ~IBedAllocator() = default;
};

// Interface Aggregation: IBedManager combines both IBedCapacity and IBedAllocator
// This allows a class to implement both bed capacity and bed allocation functionality at once
class IBedManager : public IBedCapacity, public IBedAllocator {}; // Aggregated interface

// General Bed Manager (LSP Compliant) - Manages general beds
class GeneralBedManager : public IBedManager {  // Implements both IBedCapacity and IBedAllocator
protected:
    int capacity;
public:
...
// Specialized ICU Bed Manager (LSP Applied) - Manages ICU beds
class ICUBedManager : public IBedManager {  // Implements both IBedCapacity and IBedAllocator
protected:
    int icuBeds;
public:
